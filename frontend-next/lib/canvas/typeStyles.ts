...
export function getStatusStyle(status?: string): { bg: string; text: string; label: string } {
  switch (status) {
    case "live":
      return { bg: "bg-emerald-100", text: "text-emerald-800", label: "live" };
    case "released":
      return { bg: "bg-sky-100", text: "text-sky-800", label: "released" };
    case "planned":
      return { bg: "bg-slate-100", text: "text-slate-600", label: "planned" };
    case "deprecated":
      return { bg: "bg-rose-100", text: "text-rose-800", label: "deprecated" };
    default:

...
import { createClassic, patchClassic, patchSovaia } from "@/app/(app)/navigator/actions";
import type {
...
export function EditDrawer({ open, mode, node, defaultPath, onClose, onSaved }: Props) {
  ...
  const llmPersona: RefinePersona = mode === "edit-sovaia" ? "architect" : "decision-maker";
        ...
        <form onSubmit={onSubmit} className="flex-1 flex flex-col">
            ...
            {mode === "edit-sovaia" && (
              <>
                ...
                <p className="text-[11px] text-slate-500 italic">
                  Edits landen als Tenant-Overlay über der Baseline. Reset über „Revert"-Aktion am Knoten möglich.
                </p>
              </>
            )}
          ...
          <footer className="px-4 py-3 border-t border-slate-200 bg-slate-50 flex items-center gap-2 justify-end">
            <button type="button" onClick={onClose}
              className="px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100 rounded-md" disabled={submitting}>
              Abbrechen
            </button>

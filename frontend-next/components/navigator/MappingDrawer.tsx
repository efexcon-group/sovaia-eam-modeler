...
import { createMapping, deleteMapping, patchMapping } from "@/app/(app)/navigator/actions";
import type { MappingCreatePayload, NavigatorMapping, NavigatorNode } from "@/lib/modeler-types";
...
export function MappingDrawer({
  ...
  const buildPayload = (): MappingCreatePayload => {
    ...
    const stripUndef = (o: Record<string, unknown>) =>
      Object.fromEntries(Object.entries(o).filter(([, v]) => v !== undefined));
    const vorher = stripUndef({ capex: num(vorherCapex), "opex-monatlich": num(vorherOpex), annahmen: vorherAnn || undefined });
    const nachher = stripUndef({ capex: num(nachherCapex), "opex-monatlich": num(nachherOpex), annahmen: nachherAnn || undefined });
    return {
      "classic-node-ids": classicIds,
      "sovaia-node-ids": sovaiaIds,
      "narrative-de": narrative,
      vorher: Object.values(vorher).length ? (vorher as MappingCreatePayload["vorher"]) : undefined,
      nachher: Object.values(nachher).length ? (nachher as MappingCreatePayload["nachher"]) : undefined,
          ...
          <div className="p-4 space-y-4 flex-1">
            ...
            <Field label="Narrativ (warum diese Transformation Sinn macht)">
              <textarea value={narrative} onChange={(e) => setNarrative(e.target.value)} rows={4} required
                placeholder="z.B. Mehrere Excel-Listen werden zu einem zentralen AI-Schicht-Optimierer konsolidiert."

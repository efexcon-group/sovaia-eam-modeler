...
import { getClusterDetail, getSovaiaReference } from "@/lib/canvas-client";
import type { ReferenceEdge, ReferenceModel, ReferenceNode } from "@/lib/modeler-types";
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    for (const m of state.models) {
      for (const n of m.nodes ?? []) {
        // Letzter gewinnt: Detail-Files überschreiben den Cluster-Anchor.
        nodeMap.set(n.id, { ...nodeMap.get(n.id), ...n });
      }
      for (const e of m.edges ?? []) edgeList.push(e);
    }
  ...
  const onNodeClick: NodeMouseHandler = useCallback(
    (_evt, node) => {
      ...
      getClusterDetail(ref)
        ...
        .catch((e) => setState((s) => ({ ...s, error: String(e), loadingCluster: null })));
    },
    [state.models],
  );
  ...
  return (
    <div className="h-full w-full">
      <div className="absolute top-3 left-3 z-10 rounded-md bg-white shadow-sm border border-slate-200 px-3 py-2 text-xs text-slate-600">
        <div className="font-medium text-slate-900">Architekt-Canvas</div>
        ...
        <div className="mt-0.5 text-slate-400">Klick auf orangenes „Detail laden" öffnet Verticals / Stacks.</div>
      </div>
